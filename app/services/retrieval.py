"""Shared vector retrieval (FAISS / DB fallback) for search and study tools."""
...
async def retrieve_context(request: "Request", query: str, top_k: int | None = None):
    """
    Returns (results, context_blocks_joined_or_empty).
    results: list of chunk/doc hit dicts (same shape as /search).
    """
    if top_k is None:
